...
use switchboard_on_demand::{
    default_queue, on_demand::oracle_quote::QuoteBuilder, QUOTE_PROGRAM_ID,
};
...
fn test_oracle_integration_with_litesvm() {
    ...
    // Load the program
    let program_id = Pubkey::from(crank_oracle::ID.to_bytes());
    ...
    let feed_bytes: [u8; 32] = [
        0xef, 0x0d, 0x8b, 0x6f, 0xcd, 0x01, 0x04, 0xe3, 0xe7, 0x50, 0x96, 0x91, 0x2f, 0xc8, 0xe1,
        0xe4, 0x32, 0x89, 0x3d, 0xa4, 0xf1, 0x8f, 0xae, 0xda, 0xac, 0xca, 0x7e, 0x58, 0x75, 0xda,
        0x62, 0x0f,
    ];
    ...
    let quote_account = Pubkey::new_from_array(
        quote
            .canonical_key(&queue_key, &Pubkey::from(QUOTE_PROGRAM_ID.to_bytes()))
            .to_bytes(),
    );
    ...
    let account_info = svm
        .get_account(&quote_account)
        .expect("Account should exist");
    println!(
        "   ✓ Quote account created with {} bytes, owner: {}",
    ...
    println!(
        "   ⏰ Advanced to slot: {} (quote is {} slots old)",
        current_slot,
        current_slot - quote_slot
    );
    ...
    let program_id = crank_oracle::ID;
    ...
    let instruction = Instruction {
        program_id,
        accounts,
        data: crank_oracle::instruction::ReadOracleData {}.data(),
    };
...
fn test_multiple_feeds() {
    ...
    let eth_feed: [u8; 32] = [
        0x84, 0xc2, 0xdd, 0xe9, 0x63, 0x3d, 0x93, 0xd1, 0xbc, 0xad, 0x84, 0xe7, 0xdc, 0x41, 0xc9,
        0xd5, 0x65, 0x78, 0xb7, 0xec, 0x52, 0xfa, 0xbe, 0xdc, 0x1f, 0x33, 0x5d, 0x67, 0x3d, 0xf0,
        0xa7, 0xc1,
    ];
    ...
    let instruction = Instruction {
        program_id: Pubkey::from(crank_oracle::ID.to_bytes()),
        accounts,
        data: crank_oracle::instruction::ReadOracleData {}.data(),
    };
...
fn test_price_extremes() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            panic!(
                "❌ Should handle extreme price values. Transaction failed: {:?}",
                e
            );
        }
    }
