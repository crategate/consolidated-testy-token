...
- [ ] **Bounty USD conversion is POOL-priced** — `permissionless_crank` pays `usd_raw × 1e6 / pool_price`, so the payout's REAL value scales by market_price / pool_price. Devnet's drifted pool ($1,232/SOL) pays ~$0.06 of real value per $0.50 bounty; on mainnet (canonical pool ≈ market) it pays the real $0.50. Tune by hardcoding `BOUNTY_USD_RAW` in `scripts/init-bounty.ts` (USDC raw, 6 dp) before `anchor run init`, or post-init via the authority-gated `set_bounty_usd` crank-oracle instruction (no script — a one-line client call; the +5%/yr inflation compounds on whatever is set). $0.50/transition at ~2–6 transitions/day vs ~3–5 crank txs + the keeper's data-API costs is the keeper-economics dial — revisit when a second keeper instance exists.
