...
## 2. Remove devnet-only code — mock-dex-pool + mock oracles REMOVED (2026-09-03); remaining: state drivers + localnet test migration

- [x] `programs/mock-dex-pool` — **DELETED 2026-09-03** (program dir, workspace dep, Anchor.toml entries, build artifacts). `execute_swap` is CPMM-only; the mock fallback branch and the mock `pool_state`/`pool_afho`/`pool_usdc`/`pool_sol`/`dex_program` accounts are stripped from `dex_buyback`/`distribute_staker_rewards`/`buy_the_dip`; `set_price`/`mock_price` PDAs are no longer read by any instruction or client.
- [x] Mock price reads — **REMOVED 2026-09-03**: `read_price`'s mock branch, `read_live_price`, and the `spot_oracle`/`sol_oracle`/`price_oracle` accounts are gone from `offer_claim`, `offer_claim_sol`, `calc_completed_offers`, `update_tradeday_stats`, `make_offers`, `load_offers`. Every priced instruction now requires the pinned CPMM (`PoolNotPinned` otherwise) and reads the pool TWAP/vault-ratio. Frontend `computeLivePrice` is pool-only too (no raw stub fallbacks).
...
- [ ] **Localnet test suite migration (blocking the suite, not mainnet).** The mock-dependent tests (`tests/legacy-mock/`: offer-claim, dex-buyback, buy-the-dip, ratchet-decay + 2 repro/smoke scripts) exercise the deleted mock venue/price paths and no longer compile/run. Port them to a pinned-CPMM harness (localnet Raydium CPMM program + seeded pool, or a Surfpool mainnet fork) before the next full-suite run. `tests/staking.test.ts` is mock-free and unaffected.

## 3. Real price oracles — POOL-ONLY (mock oracles deleted 2026-09-03)
...
- [x] **H1 re-pin for CPMM** — the CPMM accounts were `UncheckedAccount` (validated only by the CPI); now verified against the derived vault/observation/authority PDAs on every swap/pricing instruction (`pinned_pool_accounts_valid` / `pinned_sol_usdc_accounts_valid` / `require_pinned_pricing_accounts` — the pin is REQUIRED since 2026-09-03: `PoolNotPinned` when unset), so a compromised keeper can't redirect the pricing reads or swap in/out vaults.
...
- [ ] Remove the now-dead `sol_vault`/`sol_dip`/`sol_rewards`/`sol_oracle` state fields + accounts (struct accounts + instruction reads removed 2026-09-03; the AmmState FIELDS + initialize accounts remain — unread, §4 final pass).
