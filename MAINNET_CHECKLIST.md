...
- [ ] **`initialize_state` writes `current_state = 99`** (crank-oracle) — outside the documented 0–3 mapping; consumers fail closed today, but either clamp to a valid state or document the sentinel in AGENTS.md.
- [ ] **`max_multiplier_bps` uncapped** in `staking::initialize_pool` (u16, header claims 1.0x–3.0x) — add a `<= 30_000`-style cap to match the spec.
- [ ] **`create_amm_position` accepts index gaps** (`next_index.max(index + 1)`) — only the AMM can call it, so severity is LOW; consider `require!(index == next_index)` for parity with `stake`.
