...
- [x] **Stale e3 mock SOL price (found 2026-09-03, launch-blocking for SOL claims).** The wSOL `mock_price` PDA (`b"mock_price"` + wSOL under mock-dex-pool, a GLOBAL PDA that survives re-inits) was last seeded at 200_000 — the pre-e9 milli-USD scale for 200 USDC/SOL. Any stack reading the fallback (fresh AmmState without re-run `set-sol-usdc-pool`) prices SOL 1000× small → the desk's SOL quote showed 1e6× large (10.39 USDC → "52,000 SOL"), and the on-chain SOL claim leg would demand an impossible lamports amount. Fix: `amm-test-data` now re-seeds the wSOL mock at 200_000_000_000 (200 × 1e9, env `MOCK_SOL_PRICE`) on every run. Ops reminder: after every `amm-init`, re-run `set-cpmm-pool` + `set-sol-usdc-pool` — the pool pins live in AmmState and a fresh state has none, which is what forces the mock fallback in the first place. Scenario sweep tool: `sim/sheet_scenarios.py` (exact combinator port, 15 labeled market states).
