...
| D | crank-oracle native rewrite (298 KB → ~60–100 KB target); drop the unused `anchor-spl`/`token_2022` deps (dead weight today) | **−1.0 to −1.25 SOL** | 528-line program; SBOD CPI via native API. Re-audit. |
...
Rolling totals (@5,080): A+C (landed) → **~6.3 SOL** · +B → **~5.9–6.1 SOL** · +D or E → **~4.8–5.2 SOL** · +F → **~3.1–3.7 SOL** ·
+G → **~1.1–2.0 SOL**. Compression adds $0 to this bill. (At mainnet's current 6,334 rate the same binaries are ~8.7 SOL → ~7.9 SOL after A+C.)
