...
// NOTE(vendor patch): OracleAccountData is ~4.8KB — larger than the 4KB SBF
// frame limit. Compiling its anchor AccountDeserialize impl into an on-chain
// build always trips the toolchain stack check ("Stack offset ... exceeded
// max offset of 4096") and any runtime call would fault in the BPF VM. The
// impl is only needed by host-side client code, so gate it on `client`
// instead of `anchor-lang`.
cfg_client! {
